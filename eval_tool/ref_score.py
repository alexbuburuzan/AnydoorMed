...
@torch.no_grad()
def calculate_dinov2_score_given_paths(path_ref, path_pred, batch_size, device, num_workers):
    model = FrozenDinoV2Encoder()
    model.eval()
    model.to(device)

    # Create transforms without normalization since DINOv2 handles it internally
    transforms = torchvision.transforms.Compose([
        torchvision.transforms.Resize((224, 224)),
        torchvision.transforms.ToTensor()
    ])

    dataset = ImagePathsDataset(path_ref, path_pred, transforms)
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, num_workers=num_workers
    )
    ...
    for object_ref, object_pred in tqdm(dataloader):
        ...
        ref_feat = model(object_ref)
        pred_feat = model(object_pred)
...
def main():
    ...
    dinov2_value = calculate_dinov2_score_given_paths(
        args.path_ref, args.path_pred, args.batch_size, device, num_workers
    )

    print('CLIP: ', clip_value)
    print('DINOv2: ', dinov2_value)
